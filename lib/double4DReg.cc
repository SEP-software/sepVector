...
std::shared_ptr<double4DReg> double4DReg::clone() const {
  if (getSpaceOnly()) {
    std::shared_ptr<double4DReg> x(new double4DReg(getHyper()));

    return x;
  } else {
    std::shared_ptr<double4DReg> x(new double4DReg(getHyper(), *_mat));

    return x;
  }
}
std::shared_ptr<double4DReg> double4DReg::cloneSpace() const {
  std::shared_ptr<double4DReg> x(new double4DReg(getHyper()));
  x->_mat = 0;
...
void double4DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
  ...
  _mat.reset(
      new double4D(boost::extents[axes[3].n][axes[2].n][axes[1].n][axes[0].n]));
  setData(_mat->data());
}
void double4DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
                           const double4D &vals) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);
  if (axes.size() != 4)
...
std::shared_ptr<double4DReg> double4DReg::window(
  ...
  std::shared_ptr<double4DReg> out(new double4DReg(hypOut));
