...
std::shared_ptr<int5DReg> int5DReg::clone() const {
  if (getSpaceOnly()) {
    std::shared_ptr<int5DReg> x(new int5DReg(getHyper()));

    return x;
  } else {
    std::shared_ptr<int5DReg> x(new int5DReg(getHyper(), *_mat));

    return x;
  }
}
std::shared_ptr<int5DReg> int5DReg::cloneSpace() const {
  std::shared_ptr<int5DReg> x(new int5DReg(getHyper()));
  x->_mat = 0;
...
void int5DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
  ...
  _mat.reset(new int5D(
      boost::extents[axes[4].n][axes[3].n][axes[2].n][axes[1].n][axes[0].n]));
  setData(_mat->data());
}
void int5DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
                        const int5D &vals) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);
...
std::shared_ptr<int5DReg> int5DReg::window(const std::vector<int> &nw,
  ...
  std::shared_ptr<int5DReg> out(new int5DReg(hypOut));
