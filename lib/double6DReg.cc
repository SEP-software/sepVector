...
std::shared_ptr<double6DReg> double6DReg::clone() const {
  if (getSpaceOnly()) {
    std::shared_ptr<double6DReg> x(new double6DReg(getHyper()));

    return x;
  } else {
    std::shared_ptr<double6DReg> x(new double6DReg(getHyper(), *_mat));

    return x;
  }
}
std::shared_ptr<double6DReg> double6DReg::cloneSpace() const {
  std::shared_ptr<double6DReg> x(new double6DReg(getHyper()));
  x->_mat = 0;
...
void double6DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);

  assert(axes.size() == 6);

  _mat.reset(new double6D(boost::extents[axes[5].n][axes[4].n][axes[3].n]
                                        [axes[2].n][axes[1].n][axes[0].n]));
  setData(_mat->data());
}
void double6DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
                           const double6D &vals) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);
...
std::shared_ptr<double6DReg> double6DReg::window(
  ...
  std::shared_ptr<double6DReg> out(new double6DReg(hypOut));
