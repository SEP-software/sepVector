...
std::shared_ptr<complexDouble7DReg> complexDouble7DReg::clone() const {
    if (getSpaceOnly()) {
        std::shared_ptr<complexDouble7DReg> x(new complexDouble7DReg(getHyper()));

        return x;
    }
    else {
        std::shared_ptr<complexDouble7DReg> x(new complexDouble7DReg(getHyper(), *_mat));

        return x;
    }
}
std::shared_ptr<complexDouble7DReg> complexDouble7DReg::cloneSpace() const {
    std::shared_ptr<complexDouble7DReg> x(new complexDouble7DReg(getHyper()));
    x->_mat = 0;
...
void complexDouble7DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
    ...
    _mat.reset(new complexDouble7D(boost::extents[axes[6].n][axes[5].n][axes[4].n][axes[3].n]
        [axes[2].n][axes[1].n][axes[0].n]));
    setData(_mat->data());
}
void complexDouble7DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
    const complexDouble7D &vals) {
    const std::vector<SEP::axis> axes = hyp->getAxes();
    setHyper(hyp);
...
std::shared_ptr<complexDouble7DReg> complexDouble7DReg::window(
    ...
    std::shared_ptr<complexDouble7DReg> out(new complexDouble7DReg(hypOut));
                    ...
                    for (int i2 = 0; i2 < nw[2]; i2++) {
                        for (int i1 = 0; i1 < nw[1]; i1++) {
                            for (int i0 = 0; i0 < nw[0]; i0++) {
                                (*out->_mat)[i6][i5][i4][i3][i2][i1][i0] =
                                    (*_mat)[fw[6] + i6 * jw[6]][fw[5] + i5 * jw[5]][fw[4] + i4 * jw[4]]
