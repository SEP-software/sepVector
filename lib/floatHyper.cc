...
void floatHyper::add(const std::shared_ptr<Vector> vec2){
  ...
  std::shared_ptr<floatHyper> vec2H = std::dynamic_pointer_cast<floatHyper>( vec2);



  for(long long i=0; i < _hyper->getN123(); i++) _vals[i]+=vec2H->_vals[i];
}
void floatHyper::scaleAdd(const double sc1, std::shared_ptr<Vector> vec2, const double sc2){
    assert(checkSame(vec2,true));
  std::shared_ptr<floatHyper> vec2H = std::dynamic_pointer_cast<floatHyper>( vec2);

  for(long long i=0; i < _hyper->getN123(); i++) _vals[i]=_vals[i]*sc1+sc2*vec2H->_vals[i];
}
 ...
 void floatHyper::random(){
  assert(!spaceOnly());
  for(long long i=0; i < _hyper->getN123(); i++) _vals[i]=((double) rand() / (RAND_MAX))-.5;
...
float floatHyper::absMax() const {
   ...
   for(int i=1; i < _hyper->getN123(); i++) val=std::max(val,fabsf(_vals[i]));
   return val;
}
float floatHyper::min() const {
   float val=fabsf(_vals[0]);
   for(int i=1; i < _hyper->getN123(); i++) val=std::min(val,fabsf(_vals[i]));
   return val;
}
...
bool floatHyper::checkSame(const std::shared_ptr<Vector> vec2, bool checkAlloc) const{
