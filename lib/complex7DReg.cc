...
std::shared_ptr<complex7DReg> complex7DReg::clone() const {
    if (getSpaceOnly()) {
        std::shared_ptr<complex7DReg> x(new complex7DReg(getHyper()));

        return x;
    }
    else {
        std::shared_ptr<complex7DReg> x(new complex7DReg(getHyper(), *_mat));

        return x;
    }
}
std::shared_ptr<complex7DReg> complex7DReg::cloneSpace() const {
    std::shared_ptr<complex7DReg> x(new complex7DReg(getHyper()));
    x->_mat = 0;
...
void complex7DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
    ...
    _mat.reset(new complex7D(boost::extents[axes[6].n][axes[5].n][axes[4].n][axes[3].n]
        [axes[2].n][axes[1].n][axes[0].n]));
    setData(_mat->data());
}
void complex7DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
    const complex7D &vals) {
    const std::vector<SEP::axis> axes = hyp->getAxes();
    setHyper(hyp);
    ...
    if (axes[5].n != vals.shape()[0])
        throw(SEPException(std::string("Axis 7 not the same (") +
            std::to_string(axes[6].n) + std::string(",") +
...
std::shared_ptr<complex7DReg> complex7DReg::window(
    ...
    std::shared_ptr<complex7DReg> out(new complex7DReg(hypOut));
