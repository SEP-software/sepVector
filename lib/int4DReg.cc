...
std::shared_ptr<int4DReg> int4DReg::clone() const {
  if (getSpaceOnly()) {
    std::shared_ptr<int4DReg> x(new int4DReg(getHyper()));

    return x;
  } else {
    std::shared_ptr<int4DReg> x(new int4DReg(getHyper(), *_mat));

    return x;
  }
}
std::shared_ptr<int4DReg> int4DReg::cloneSpace() const {
  std::shared_ptr<int4DReg> x(new int4DReg(getHyper()));
  x->_mat = 0;
...
void int4DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
  ...
  _mat.reset(
      new int4D(boost::extents[axes[3].n][axes[2].n][axes[1].n][axes[0].n]));
  setData(_mat->data());
}
void int4DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
                        const int4D &vals) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);
...
std::shared_ptr<int4DReg> int4DReg::window(const std::vector<int> &nw,
  ...
  std::shared_ptr<int4DReg> out(new int4DReg(hypOut));
