...
std::shared_ptr<complex5DReg> complex5DReg::clone() const {
  if (getSpaceOnly()) {
    std::shared_ptr<complex5DReg> x(new complex5DReg(getHyper()));

    return x;
  } else {
    std::shared_ptr<complex5DReg> x(new complex5DReg(getHyper(), *_mat));

    return x;
  }
}
std::shared_ptr<complex5DReg> complex5DReg::cloneSpace() const {
  std::shared_ptr<complex5DReg> x(new complex5DReg(getHyper()));
  x->_mat = 0;
...
void complex5DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
  ...
  _mat.reset(new complex5D(
      boost::extents[axes[4].n][axes[3].n][axes[2].n][axes[1].n][axes[0].n]));
  setData(_mat->data());
}
void complex5DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
                            const complex5D &vals) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);
