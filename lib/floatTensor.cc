...
void floatTensor::add(const std::shared_ptr<floatTensor> vec2)
{
  if (!checkSame(vec2))
    throw(std::string("Vectors not of the same space"));
  std::shared_ptr<floatTensor> vec2H =
      std::dynamic_pointer_cast<floatTensor>(vec2);
...
void floatTensor::mult(const std::shared_ptr<floatTensor> vec2)
{
  if (!checkSame(vec2))
    throw(std::string("Vectors not of the same space"));
  std::shared_ptr<floatTensor> vec2H =
...
void floatTensor::clipVector(const std::shared_ptr<floatTensor> beg,
                            const std::shared_ptr<floatTensor> end)
{
  if (!checkSame(beg))
    throw(std::string("Vectors not of the same space"));
  if (!checkSame(end))
  ...
  std::shared_ptr<floatTensor> begH = std::dynamic_pointer_cast<floatTensor>(beg);
  std::shared_ptr<floatTensor> endH = std::dynamic_pointer_cast<floatTensor>(end);
  tbb::parallel_for(tbb::blocked_range<long long>(0, getHyper()->getN123()),
                    [&](const tbb::blocked_range<long long> &r) {
                      for (long long i = r.begin(); i != r.end(); ++i)
                        v[i] = std::min(ev[i],
...
void floatTensor::scaleAdd(std::shared_ptr<floatTensor> vec2, const double sc1,
                          const double sc2)
{
  if (!checkSame(vec2))
    throw(std::string("Vectors not of the same space"));
...
float floatTensor::cent(const long long iv, const int js) const
{
...
void floatTensor::clip(const float bclip, const float eclip, bool outer)
{
  if (spaceOnly())
...
void floatTensor::scale(double sc)
{
  if (spaceOnly())
    throw(std::string("Vectors not allocated"));
      float *v=getVals();
...
double floatTensor::norm(const int n) const
{
...
void floatTensor::set(const float val)
{
    float *v=getVals();
...
void floatTensor::calcHisto(std::shared_ptr<int1DReg> &histo, float mn,
                           float mx)
{
...
double floatTensor::dot(const std::shared_ptr<floatTensor> vec2) const
{
  if (!checkSame(vec2))
    throw(std::string("Vectors not of the same space"));
  std::shared_ptr<floatTensor> vec2H =
...
void floatTensor::createMask(const float zero, const float err)
{
...
void floatTensor::infoStream(const int lev, std::stringstream &x)
{
...
void floatTensor::softClip(const float scale)
{
...
double floatTensor::absMax() const
{
    const float *vs=getCVals();
...
bool floatTensor::checkSame(const std::shared_ptr<floatTensor> vec2) const
{
