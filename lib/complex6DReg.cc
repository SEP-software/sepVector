...
std::shared_ptr<complex6DReg> complex6DReg::clone() const {
  if (getSpaceOnly()) {
    std::shared_ptr<complex6DReg> x(new complex6DReg(getHyper()));

    return x;
  } else {
    std::shared_ptr<complex6DReg> x(new complex6DReg(getHyper(), *_mat));

    return x;
  }
}
std::shared_ptr<complex6DReg> complex6DReg::cloneSpace() const {
  std::shared_ptr<complex6DReg> x(new complex6DReg(getHyper()));
  x->_mat = 0;
...
void complex6DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);

  assert(axes.size() == 6);

  _mat.reset(new complex6D(boost::extents[axes[5].n][axes[4].n][axes[3].n]
                                         [axes[2].n][axes[1].n][axes[0].n]));
  setData(_mat->data());
}
void complex6DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
                            const complex6D &vals) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);
...
std::shared_ptr<complex6DReg> complex6DReg::window(
  ...
  std::shared_ptr<complex6DReg> out(new complex6DReg(hypOut));
