...
*/
  class longHyper : public regSpace
  {
  public:
    /*!
Initializer for longHyper class. Only used by inherited class
*/
    longHyper() { ; }
    /*!
  ...
  */
    void add(std::shared_ptr<longHyper> vec);
  ...
  */
    void scaleAdd(std::shared_ptr<longHyper> vec2, const double sc1,
                  const double sc2);
...
*/
   ...
   self*=vec2
*/
    void mult(std::shared_ptr<longHyper> vec2);
   ...
   */
    double dot(std::shared_ptr<longHyper> vec2) const;
...
*/
    void setData(long long *ptr)
    {
      _vals = ptr;
      setNotSpace();
      setMemPtr((void *)ptr, sizeof(long long));
    }
...
*/
    void clip(const long long bclip, const long long eclip);
...
*/
    bool isDifferent(std::shared_ptr<longHyper> vec2)
    {
      calcCheckSum();
...
*/
  ...
  */
    long long *getVals() { return _vals; }
    /*!
  Return the pointer to the memory for the vector with const tag
*/
    const long long *getCVals() const { return _vals; }
 ...
 */
    ...
    bool checkSame(const std::shared_ptr<SEP::longHyper> vec2) const;
  ...
  protected:
    void copyVals(const long long *buf)
    {
      memcpy(getVals(), buf, sizeof(long long) * getHyper()->getN123());
    }
    long long *_vals;         ///< Storage for vector
