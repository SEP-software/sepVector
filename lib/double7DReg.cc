...
std::shared_ptr<double7DReg> double7DReg::clone() const {
    if (getSpaceOnly()) {
        std::shared_ptr<double7DReg> x(new double7DReg(getHyper()));

        return x;
    }
    else {
        std::shared_ptr<double7DReg> x(new double7DReg(getHyper(), *_mat));

        return x;
    }
}
std::shared_ptr<double7DReg> double7DReg::cloneSpace() const {
    std::shared_ptr<double7DReg> x(new double7DReg(getHyper()));
    x->_mat = 0;
...
void double7DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
    ...
    _mat.reset(new double7D(boost::extents[axes[6].n][axes[5].n][axes[4].n][axes[3].n]
        [axes[2].n][axes[1].n][axes[0].n]));
    setData(_mat->data());
}
void double7DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
    const double7D &vals) {
    const std::vector<SEP::axis> axes = hyp->getAxes();
    setHyper(hyp);
                    ...
                    for (long long k = 0; k < axes[2].n; k++) {
                        for (long long j = 0; j < axes[1].n; j++) {
                            for (long long i = 0; i < axes[0].n; i++) {
                                (*_mat)[o][n][m][l][k][j][i] = vals[o][n][m][l][k][j][i];
                            }
                        }
                    }
                }
...
std::shared_ptr<double7DReg> double7DReg::window(
    ...
    std::shared_ptr<double7DReg> out(new double7DReg(hypOut));
