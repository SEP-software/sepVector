...
std::shared_ptr<Vector> float3DReg::clone() const {
        if(getSpaceOnly()) {
...
std::shared_ptr<Vector> float3DReg::cloneSpace() const {
...
void float3DReg::initData(std::shared_ptr<SEP::hypercube> hyp, const float3D &vals){
        const std::vector<SEP::axis> axes= hyp->getAxes();
        setHyper(hyp);

        assert(axes.size()==3);
        assert(axes[0].n==vals.shape()[2] && axes[1].n==vals.shape()[1] && axes[2].n==vals.shape()[0]);
        _mat.reset(new float3D(boost::extents[axes[2].n][axes[1].n][axes[0].n]));
        setData(_mat->data());
