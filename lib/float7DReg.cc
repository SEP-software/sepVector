...
std::shared_ptr<float7DReg> float7DReg::clone() const {
    if (getSpaceOnly()) {
        std::shared_ptr<float7DReg> x(new float7DReg(getHyper()));

        return x;
    }
    else {
        std::shared_ptr<float7DReg> x(new float7DReg(getHyper(), *_mat));

        return x;
    }
}
std::shared_ptr<float7DReg> float7DReg::cloneSpace() const {
    std::shared_ptr<float7DReg> x(new float7DReg(getHyper()));
    x->_mat = 0;
...
void float7DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
    ...
    _mat.reset(new float7D(boost::extents[axes[6].n][axes[5].n][axes[4].n][axes[3].n]
        [axes[2].n][axes[1].n][axes[0].n]));
    setData(_mat->data());
}
void float7DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
    const float7D &vals) {
    const std::vector<SEP::axis> axes = hyp->getAxes();
    setHyper(hyp);
...
std::shared_ptr<float7DReg> float7DReg::window(
    ...
    std::shared_ptr<float7DReg> out(new float7DReg(hypOut));
