...
void longHyper::add(const std::shared_ptr<longHyper> vec2)
{
  if (!checkSame(vec2))
    throw(std::string("Vectors not from the same space"));

  std::shared_ptr<longHyper> vec2H = std::dynamic_pointer_cast<longHyper>(vec2);
...
void longHyper::mult(const std::shared_ptr<longHyper> vec2)
{
  if (!checkSame(vec2))
    throw(std::string("Vectors not from the same space"));
  std::shared_ptr<longHyper> vec2H = std::dynamic_pointer_cast<longHyper>(vec2);
...
void longHyper::scaleAdd(std::shared_ptr<longHyper> vec2, const double sc1,
                        const double sc2)
{
  if (!checkSame(vec2))
    throw(std::string("Vectors not from the same space"));
...
int longHyper::cent(const long long iv, const int js) const
{
...
void longHyper::clip(const long long  bclip, const long long eclip)
{
  if (getSpaceOnly())
...
void longHyper::signum()
{
  if (getSpaceOnly())
    throw(std::string("Vector not allocated"));
...
void longHyper::scale(double sc)
{
  if (getSpaceOnly())
    throw(std::string("Vectors not allocated"));
  tbb::parallel_for(tbb::blocked_range<long long>(0, getHyper()->getN123()),
...
long long longHyper::norm(const int n) const
{
...
void longHyper::set(const int val)
{
  tbb::parallel_for(tbb::blocked_range<long long>(0, getHyper()->getN123()),
                    [&](const tbb::blocked_range<long long> &r) {
                      for (long long i = r.begin(); i != r.end(); ++i)
...
double longHyper::dot(const std::shared_ptr<longHyper> vec2) const
{
  if (!checkSame(vec2))
    throw(std::string("Vectors not from the same space"));
  std::shared_ptr<longHyper> vec2H = std::dynamic_pointer_cast<longHyper>(vec2);
...
void longHyper::createMask(const int zero, const int err)
{
  tbb::parallel_for(tbb::blocked_range<long long>(0, getHyper()->getN123()),
                    [&](const tbb::blocked_range<long long> &r) {
                      for (long long i = r.begin(); i != r.end(); ++i)
...
void longHyper::infoStream(const int lev, std::stringstream &x)
{
...
void longHyper::softClip(const float scale)
{
...
bool longHyper::checkSame(const std::shared_ptr<longHyper> vec2) const
{
  if (!vec2)
  {
