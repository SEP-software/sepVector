...
std::shared_ptr<byte6DReg> byte6DReg::clone() const {
  if (getSpaceOnly()) {
    std::shared_ptr<byte6DReg> x(new byte6DReg(getHyper()));

    return x;
  } else {
    std::shared_ptr<byte6DReg> x(new byte6DReg(getHyper(), *_mat));

    return x;
  }
}
std::shared_ptr<byte6DReg> byte6DReg::cloneSpace() const {
  std::shared_ptr<byte6DReg> x(new byte6DReg(getHyper()));
  x->_mat = 0;
...
void byte6DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
  ...
  _mat.reset(new byte6D(boost::extents[axes[5].n][axes[4].n][axes[3].n]
                                      [axes[2].n][axes[1].n][axes[0].n]));
  setData(_mat->data());
}
void byte6DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
                         const byte6D &vals) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);
...
std::shared_ptr<byte6DReg> byte6DReg::window(const std::vector<int> &nw,
  ...
  std::shared_ptr<byte6DReg> out(new byte6DReg(hypOut));
