...
std::shared_ptr<byte5DReg> byte5DReg::clone() const {
  if (getSpaceOnly()) {
    std::shared_ptr<byte5DReg> x(new byte5DReg(getHyper()));

    return x;
  } else {
    std::shared_ptr<byte5DReg> x(new byte5DReg(getHyper(), *_mat));

    return x;
  }
}
std::shared_ptr<byte5DReg> byte5DReg::cloneSpace() const {
  std::shared_ptr<byte5DReg> x(new byte5DReg(getHyper()));
  x->_mat = 0;
...
void byte5DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
  ...
  _mat.reset(new byte5D(
      boost::extents[axes[4].n][axes[3].n][axes[2].n][axes[1].n][axes[0].n]));
  setData(_mat->data());
}
void byte5DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
                         const byte5D &vals) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);
...
std::shared_ptr<byte5DReg> byte5DReg::window(const std::vector<int> &nw,
                                             const std::vector<int> &jw,
                                             const std::vector<int> &fw) const {
  const std::vector<SEP::axis> axes = getHyper()->getAxes();
  if (nw.size() != axes.size()) throw(SEPException("nw must of length 5"));
  if (fw.size() != axes.size()) throw(SEPException("fw must of length 5"));
  if (jw.size() != axes.size()) throw(SEPException("jw must of length 5"));
  ...
  std::shared_ptr<byte5DReg> out(new byte5DReg(hypOut));
