...
std::shared_ptr<byte3DReg> byte3DReg::clone() const {
  if (getSpaceOnly()) {
    std::shared_ptr<byte3DReg> x(new byte3DReg(getHyper()));
    return x;
  } else {
    std::shared_ptr<byte3DReg> x(new byte3DReg(getHyper(), *_mat));

    return x;
  }
}
std::shared_ptr<byte3DReg> byte3DReg::cloneSpace() const {
  std::shared_ptr<byte3DReg> x(new byte3DReg(getHyper()));
  x->_mat = 0;
  x->setSpace();
...
void byte3DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
...
void byte3DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
                         const byte3D &vals) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);
...
std::shared_ptr<byte3DReg> byte3DReg::window(const std::vector<int> &nw,
  ...
  std::shared_ptr<byte3DReg> out(new byte3DReg(hypOut));
